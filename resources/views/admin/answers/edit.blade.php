    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.answers.update", [$answer->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('order') ? 'is-invalid' : '' }}" type="number" name="order" id="order" value="{{ old('order', $answer->order) }}" step="1">
                @if($errors->has('order'))
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('correct') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="correct" id="correct" value="1" {{ $answer->correct || old('correct', 0) === 1 ? 'checked' : '' }}>
