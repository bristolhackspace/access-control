...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.induction.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.inductions.update", [$induction->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $induction->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('machine') ? 'is-invalid' : '' }}" name="machine_id" id="machine_id" required>
                    @foreach($machines as $id => $entry)
                        <option value="{{ $id }}" {{ (old('machine_id') ? old('machine_id') : $induction->machine->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('inductor') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="inductor" id="inductor" value="1" {{ $induction->inductor || old('inductor', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('inducted_by') ? 'is-invalid' : '' }}" name="inducted_by_id" id="inducted_by_id">
                    @foreach($inducted_bies as $id => $entry)
                        <option value="{{ $id }}" {{ (old('inducted_by_id') ? old('inducted_by_id') : $induction->inducted_by->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
