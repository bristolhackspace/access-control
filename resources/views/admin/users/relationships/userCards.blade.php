    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class=" table table-bordered table-striped table-hover datatable datatable-userCards">
                    <thead>
                        <tr>
                            <th width="10">
                    ...
                    <tbody>
                        @foreach($cards as $key => $card)
                            <tr data-entry-id="{{ $card->id }}">
                                ...
                                <td>
                                    <span style="display:none">{{ $card->active ?? '' }}</span>
                                    <input type="checkbox" disabled="disabled" {{ $card->active ? 'checked' : '' }}>
                                </td>
                                <td>
                                    <span style="display:none">{{ $card->paid_for ?? '' }}</span>
...
@endcan
  ...
  let table = $('.datatable-userCards:not(.ajaxTable)').DataTable({ buttons: dtButtons })
