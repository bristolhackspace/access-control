        ...
        <div class="col-md-12">
            @can('answer_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.answers.create') }}">
                            ...
                            <tbody>
                                @foreach($answers as $key => $answer)
                                    <tr data-entry-id="{{ $answer->id }}">
                                        ...
                                        <td>
                                            <span style="display:none">{{ $answer->correct ?? '' }}</span>
                                            <input type="checkbox" disabled="disabled" {{ $answer->correct ? 'checked' : '' }}>
                                        </td>
                                        <td>
                                            @can('answer_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.answers.show', $answer->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('answer_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.answers.edit', $answer->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('answer_delete')
                                                <form action="{{ route('frontend.answers.destroy', $answer->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('answer_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.answers.massDestroy') }}",
