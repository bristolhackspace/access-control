        ...
        <div class="col-md-12">
            @can('login_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.logins.create') }}">
                            ...
                            <tbody>
                                @foreach($logins as $key => $login)
                                    <tr data-entry-id="{{ $login->id }}">
                                        ...
                                        <td>
                                            @can('login_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.logins.show', $login->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('login_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.logins.edit', $login->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('login_delete')
                                                <form action="{{ route('frontend.logins.destroy', $login->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('login_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.logins.massDestroy') }}",
