        ...
        <div class="col-md-12">
            @can('machine_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.machines.create') }}">
                            ...
                            <tbody>
                                @foreach($machines as $key => $machine)
                                    <tr data-entry-id="{{ $machine->id }}">
                                        ...
                                        <td>
                                            @can('machine_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.machines.show', $machine->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('machine_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.machines.edit', $machine->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('machine_delete')
                                                <form action="{{ route('frontend.machines.destroy', $machine->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('machine_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.machines.massDestroy') }}",
